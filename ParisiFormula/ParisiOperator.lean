...
PROVENANCE (ParisiFormula project):
  Ported from `port/ParisiOperator.lean`, itself copied verbatim from
  or4nge19/SpinGlass (see docs/PROVENANCE.md), which targets Lean v4.28.0-rc1.
  This project is on v4.32.1.

  Port changes: added `import Mathlib.MeasureTheory.Integral.Prod` (needed for
  `MeasureTheory.integral_prod`, which `Integral.Bochner.Basic` does not pull in),
  plus whatever renames the 4.28 -> 4.32 window required.  The mathematical content
  is unchanged.

  This is Phase 2, item 1 of docs/ROADMAP.md.
-/
