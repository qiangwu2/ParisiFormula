...
PROVENANCE (ParisiFormula project):
  Ported from `port/GuerraIBP.lean`, itself copied verbatim from
  or4nge19/SpinGlass (see docs/PROVENANCE.md), which targets Lean v4.28.0-rc1.
  This project is on v4.32.1.  Port change: imports rewritten to
  `Lemmas.SpinGlass.*` / `ParisiFormula.*`, plus any 4.28 -> 4.32 renames.

  NOTE ON SCOPE: this file concerns Guerra's comparison of the SK disorder against a
  `SimpleDisorder` reference at a *single* size N (the path `H_t`).  That is
  Milestone 3 at k = 0 (the replica-symmetric Guerra bound), NOT the
  Guerra-Toninelli superadditivity path of `ParisiFormula/GuerraToninelli.lean`,
  which compares size N+M against N (+) M.  See docs/ROADMAP.md.
-/
import ParisiFormula.GuerraInterpolation
