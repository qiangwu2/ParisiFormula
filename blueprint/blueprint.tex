...
checked at every level with a positive left overlap gap, including the terminal
interval. Its zero-first-overlap extension, remaining overlap/sign cases and uniform
...
\end{proof}
\textbf{Still open:} the first-overlap-zero curvature extension.
...
The checked zero-variance identities identify $Q_1(0)$ and $R_1(0)$ with
the squares of the genuine scalar first and second derivatives. Stationarity
forces the first derivative to vanish at $q_1=0$, including when $k=0$.
The remaining curvature inequality requires a one-sided right variation;
its unequal inner and outer masses prevent direct use of the baseline
$Q'=-R$ identity.
\lean{SpinGlass.Targets.section4THessianSquare\_initial\_zero\_eq\_sq}\leanok
\lean{SpinGlass.Targets.parisiFDeriv\_initial\_zero\_of\_min}\leanok
