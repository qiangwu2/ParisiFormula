...
estimates (Propositions~5.1 and~5.3) are checked. The local-right estimate is
checked at nonterminal levels with a positive left overlap gap. Its terminal
and zero-first-overlap extensions, remaining overlap/sign cases and uniform
assembly into Theorem~2.4 remain open.
...
\begin{lemma}[The local-right estimate by reflection; stated nonboundary scope]
\lean{SpinGlass.Targets.section5RightV\_baseline\_eq\_reflected\_next}\leanok
\lean{SpinGlass.Targets.section4THessianSquare\_neighbor\_full\_eq\_zero}\leanok
\lean{SpinGlass.Targets.constrainedPhi\_local\_right\_uniform}\leanok
For $1\le r\le k$, put $a=\beta^2(q_{r+1}-q_r)$ and define the actual
reflected factors $Q_+(v)=Q_{r+1}(a-v)$ and $R_+(v)=R_{r+1}(a-v)$,
with baseline mass $m_r$. The entire right baseline lambda family agrees
with the next left family at variance $a-v$. Moreover,
\[
 Q_+'(v)=R_+(v),\qquad
 |R_+(v)-R_+(w)|\le535|v-w|,
\]
...
\end{lemma}
\begin{proof}
The paired mass arrays and sharing cutoffs agree exactly; only the split
...
\end{proof}
\textbf{Still open:} the terminal right interval and first-overlap-zero
curvature extension. In the Lean indexing $q_{k+2}=1$, not $q_{k+1}=1$,
so the terminal right interval cannot be discarded as a singleton.
The positive-left-gap curvature theorem also cannot be applied at $q_1=0$.
Thus unrestricted Proposition~5.2 is not claimed here.
...
estimate is checked away from its terminal and zero-first-overlap boundaries.
Extend it to those boundaries, then complete the remaining far-overlap and
signed cases and their uniform assembly. The scalar stationarity reduction is
