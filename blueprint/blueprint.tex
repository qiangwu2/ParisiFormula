...
The compact uniform quadratic bound of Theorem~2.4 remains open.
The results above do not yet join different trial indices at their shared
breakpoints or assemble every signed and terminal domain with the local
quadratic bounds. No continuity is asserted for the changing finite-size
overlap constraint.
...
\]
This is only the overlap component of the adjacent-trial gluing; equality of
the sorted tagged Gaussian operators still requires deleting the changed
zero-variance interpolation tag and comparing the surviving stable sorts.
\end{lemma}
...
positive. Mathlib's stable insertion-sort permutation lemma now permits
deleting one tagged entry, including equal mass ties. The scalar-list
zero-variance filter and reverse-index bridge are also checked as supporting
infrastructure. The remaining work is the reindexing between the two
adjacent tagged trial types and the terminal padding case.
...
Strict scalar interchange and sorting, the mixed paired recursion, sorted
scalar identification, cumulative tagged overlaps, the original-pressure
endpoint and the time-zero deficit for Proposition~5.7 are checked.
The actual mixed-pressure derivative inequality and endpoint transport are
now checked. Complete the remaining final-trial-interval and first-level
