...
Proposition~5.7 itself remains open. The actual simultaneous mixed-pressure
derivative must still be assembled from these contributions and identified
with the signed covariance expression before applying closed-interval
transport. In particular, separate partial derivatives alone do not justify
this chain rule. The final trial interval, positive-overlap breakpoint
selection, and first-physical-level sign cases still require boundary assembly.
The separate physical-time-zero estimate above already covers every attainable
overlap. The component lemmas are not the full Proposition~5.7 inequality.
