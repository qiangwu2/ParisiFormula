...
endpoint Hessian calculus, cubic Taylor and sixth-root curvature deduction are
checked, the latter two conditional on an explicit Lipschitz bound for the
actual Hessian-square factor. Its depth-uniform bound (Lemma~4.9), unconditional
Proposition~4.10 and the remaining Section~5 overlap estimates are still open.
...
Proposition~4.10 is proved only conditional on actual Hessian-square regularity.
...
\end{proof}
\textbf{Unproved input:} a Lipschitz constant for the actual $R$ depending only
on $\beta$, uniformly in depth and on the full interval including zero.
Continuity and the available $1/\sqrt v$ third-spatial bound do not supply it.
Thus Lemma~4.9 and unconditional Proposition~4.10 remain open; the formalised
conditional deduction does not assert that this input has been constructed.
...
mass/interior-overlap reduction. The next analytic input is the depth-uniform
Lipschitz bound for the actual Hessian-square factor. The local quadratic,
initial/dual and remaining overlap/sign estimates in Section~5 must then be
completed and assembled uniformly. The scalar stationarity reduction is
