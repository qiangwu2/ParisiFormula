...
noncomputable def mixedConstrainedCascadePathD (U : ℝ → EnergySpace n) (u : ℝ)
    (mode : ℕ → Section5GaussianMode) (m : ℕ → ℝ) (v : ℕ → ℝ → ℝ) (j : ℕ) (x y : Fin n → ℝ) (w : ℝ) : ℝ :=
...
theorem mixedConstrainedCascade_amplitude_path_anchored_bound
    (a : ℝ → ℝ) (u : ℝ) [Nonempty (AT.ConstrainedPair n u)]
    (mode : ℕ → Section5GaussianMode) (m : ℕ → ℝ) (v : ℕ → ℝ → ℝ) (hm : ∀ l, 0 ≤ m l) (j : ℕ) {w : ℝ}
    ...
    ∃ A B : ℝ, 0 ≤ A ∧ 0 ≤ B ∧ ∀ᶠ z in 𝓝 w,
      ...
      |mixedVectorCascade n mode m (fun l => v l z)
          (constrainedPairFieldBase n (a z • U) u) j x y -
        mixedVectorCascade n mode m (fun l => v l w)
  ...
  let A := ∑ l : Fin j, mixedLevelHeatBound n mode m l * Cv l
  let B := 2 * Fintype.card (Config n) * Ca
  refine ⟨A, B, Finset.sum_nonneg (fun l _ =>
    mul_nonneg (mixedLevelHeatBound_nonneg n mode m l) (hCv l).le), by positivity, ?_⟩
  filter_upwards [hba.bound, Filter.eventually_all.mpr hbv,
    Filter.eventually_all.mpr hf, hnonneg] with z haz hvz hfz hnz
  intro U x y
  have H := mixedConstrainedCascade_joint_dist_le (a w • U) (a z • U) u mode m
    (fun l => v l w) (fun l => v l z) hm hnonneg.self_of_nhds hnz j
  ...
  have hsum : (∑ l : Fin j, mixedLevelHeatBound n mode m l * |v l z - v l w|) ≤
      A * |z - w| := by
    change _ ≤ (∑ l : Fin j, mixedLevelHeatBound n mode m l * Cv l) * |z - w|
    rw [Finset.sum_mul]
    exact Finset.sum_le_sum fun l _ => by
      simpa only [mul_assoc] using mul_le_mul_of_nonneg_left (hvz l)
        (mixedLevelHeatBound_nonneg n mode m l)
...
theorem measurable_mixedConstrainedCascadePathD (a : ℝ → ℝ) (u : ℝ)
    ...
    Measurable (fun U : EnergySpace n =>
      mixedConstrainedCascadePathD (fun z => a z • U) u mode m v j x y w) := by
  let F := fun z (U : EnergySpace n) => mixedVectorCascade n mode m (fun l => max (v l z) 0)
    (constrainedPairFieldBase n (a z • U) u) j x y
  have hFm (z : ℝ) : Measurable (F z) :=
    (continuous_mixedConstrainedCascade_disorder u mode m (fun l => max (v l z) 0) hm
      (fun _ => le_max_right _ _) j x y).measurable.comp (by fun_prop)
  apply measurable_of_tendsto_metrizable' (𝓝[>] (0 : ℝ))
    (f := fun t U => t⁻¹ * (F (w + t) U - F w U))
    (fun t => ((hFm (w + t)).sub (hFm w)).const_mul t⁻¹)
  rw [tendsto_pi_nhds]
  intro U
  have H := hasDerivAt_mixedConstrainedCascade_path_fderiv (fun z => a z • U) u mode m v hm j
    (ha.smul_const U) hv hnonneg hface x y
  have H' : HasDerivAt (fun z => F z U)
      (mixedConstrainedCascadePathD (fun z => a z • U) u mode m v j x y w) w := by
...
/-- Integrability of the bottom-up mixed cascade, inherited from the proved list form. -/
theorem integrable_mixedConstrainedCascade_amplitude {Z : Ω → EnergySpace n}
    ...
    Integrable (fun ω => mixedVectorCascade n mode m v
      (constrainedPairFieldBase n (a • Z ω) u) j x y) ℙ := by
  simpa only [mixedVectorCascade_eq_list] using
    integrable_mixedConstrainedList_amplitude hZ a u mode m v (List.range j).reverse
      (fun l _ => hm l) (fun l _ => hv l) x y
...
theorem mixedConstrainedGaussian_path_derivative {Z : Ω → EnergySpace n}
    ...
    (hface : ∀ l < j, 0 < v l w ∨ (v l =ᶠ[𝓝 w] fun _ => 0))
    (x y : Fin n → ℝ) :
    Integrable (fun ω => mixedConstrainedCascadePathD
      (fun z => a z • Z ω) u mode m v j x y w) ℙ ∧
    HasDerivAt (fun z => ∫ ω, mixedVectorCascade n mode m (fun l => v l z)
      (constrainedPairFieldBase n (a z • Z ω) u) j x y ∂ℙ)
      (∫ ω, mixedConstrainedCascadePathD (fun z => a z • Z ω) u mode m v j x y w ∂ℙ) w := by
  obtain ⟨A, B, hA, hB, hb⟩ :=
    mixedConstrainedCascade_amplitude_path_anchored_bound (n := n) a u mode m v hm j ha hv hnonneg hface
  let S := {z : ℝ | (∀ l, 0 ≤ v l z) ∧
    ∀ (U : EnergySpace n) (x y : Fin n → ℝ),
      |mixedVectorCascade n mode m (fun l => v l z)
          (constrainedPairFieldBase n (a z • U) u) j x y -
  ...
  let D := fun ω => mixedConstrainedCascadePathD (fun z => a z • Z ω) u mode m v j x y w
  let L : ℝ →L[ℝ] ℝ →L[ℝ] ℝ := ContinuousLinearMap.smulRightL ℝ ℝ ℝ 1
  have he (z : ℝ) : L z 1 = z := by simp [L]
  have hDm : AEStronglyMeasurable D ℙ :=
    ((measurable_mixedConstrainedCascadePathD a u mode m v hm j ha hv hnonneg hface x y).comp
      hZ.repr_measurable).aestronglyMeasurable
  have HD (ω : Ω) : HasDerivAt (fun z => mixedVectorCascade n mode m (fun l => v l z)
      (constrainedPairFieldBase n (a z • Z ω) u) j x y) (D ω) w :=
    hasDerivAt_mixedConstrainedCascade_path_fderiv (fun z => a z • Z ω) u mode m v hm j
      (ha.smul_const (Z ω)) hv hnonneg hface x y
  have H := hasFDerivAt_integral_of_dominated_loc_of_lip'
    (μ := (ℙ : Measure Ω)) (F := fun z ω => mixedVectorCascade n mode m (fun l => v l z)
      (constrainedPairFieldBase n (a z • Z ω) u) j x y)
    (F' := fun ω => L (D ω)) hS
    (fun z hz => ((continuous_mixedConstrainedCascade_disorder u mode m (fun l => v l z) hm
      hz.1 j x y).measurable.comp (hZ.repr_measurable.const_smul (a z))).aestronglyMeasurable)
    (integrable_mixedConstrainedCascade_amplitude hZ (a w) u mode m (fun l => v l w) hm
      hnonneg.self_of_nhds j x y)
...
theorem hasDerivAt_mixedConstrainedGaussian_path {Z : Ω → EnergySpace n}
    (hZ : IsGaussianHilbert Z) (a : ℝ → ℝ) (u : ℝ)
    [Nonempty (AT.ConstrainedPair n u)]
    (mode : ℕ → Section5GaussianMode) (m : ℕ → ℝ) (v : ℕ → ℝ → ℝ) (hm : ∀ l, 0 ≤ m l) (j : ℕ) {w : ℝ}
    (ha : DifferentiableAt ℝ a w)
    ...
    HasDerivAt (fun z => ∫ ω, mixedVectorCascade n mode m (fun l => v l z)
      (constrainedPairFieldBase n (a z • Z ω) u) j x y ∂ℙ)
      (∫ ω, mixedConstrainedCascadePathD (fun z => a z • Z ω) u mode m v j x y w ∂ℙ) w :=
  (mixedConstrainedGaussian_path_derivative hZ a u mode m v hm j ha hv hnonneg hface x y).2
